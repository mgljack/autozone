  ...
  if (!lat || !lng) {
    return (
      <div className="flex h-64 items-center justify-center rounded-2xl border border-zinc-200 bg-zinc-50 text-sm text-zinc-500">
        {t("carCenter.form.selectAddress")}
      </div>
    );
  }
...
export function CarCenterForm() {
  ...
  const validate = () => {
    ...
    if (!draft.name.trim()) next.name = t("carCenter.form.error.nameRequired");
    if (!draft.phone1.trim()) next.phone1 = t("carCenter.form.error.phone1Required");
    if (!draft.hours.trim()) next.hours = t("carCenter.form.error.hoursRequired");
    if (!draft.address.trim()) next.address = t("carCenter.form.error.addressRequired");
    setErrors(next);
  ...
  const handlePaymentSuccess = () => {
    ...
    alert(t("carCenter.form.registerSuccess") || t("carCenter.form.success"));
    router.push("/service");
  };
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <div className="grid gap-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t("carCenter.form.basicInfo")}</CardTitle>
          </CardHeader>
          <CardContent className="grid gap-4">
            <div className="grid gap-2">
              <Label>{t("carCenter.form.name")}</Label>
              <Input
                value={draft.name}
                onChange={(e) => setField("name", e.target.value)}
                placeholder={t("carCenter.form.namePlaceholder")}
              />
              {errors.name ? <div className="text-xs text-red-600">{errors.name}</div> : null}
            </div>
            <div className="grid gap-2">
              <Label>{t("carCenter.form.description")}</Label>
              <Textarea
                value={draft.description}
                onChange={(e) => setField("description", e.target.value)}
                placeholder={t("carCenter.form.descriptionPlaceholder")}
                rows={4}
              />
            </div>
            <div className="grid gap-2">
              <Label>{t("carCenter.form.category")}</Label>
              <Select value={draft.category} onChange={(e) => setField("category", e.target.value)}>
                <option value="">{t("sell.common.select")}</option>
                <option value="engine">{t("carCenter.form.category.engine")}</option>
                <option value="brake">{t("carCenter.form.category.brake")}</option>
                <option value="tire">{t("carCenter.form.category.tire")}</option>
                <option value="oil">{t("carCenter.form.category.oil")}</option>
                <option value="electric">{t("carCenter.form.category.electric")}</option>
                <option value="body">{t("carCenter.form.category.body")}</option>
                <option value="other">{t("carCenter.form.category.other")}</option>
              </Select>
            </div>
            <div className="grid gap-2">
              <Label>{t("carCenter.form.services")}</Label>
              <Input
                value={draft.services.join(", ")}
                onChange={(e) => setField("services", e.target.value.split(",").map((s) => s.trim()).filter(Boolean))}
                placeholder={t("carCenter.form.servicesPlaceholder")}
              />
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t("carCenter.form.contact")}</CardTitle>
          </CardHeader>
          <CardContent className="grid gap-4">
            <div className="grid gap-2">
              <Label>{t("carCenter.form.phone1")}</Label>
              <Input
                value={draft.phone1}
                onChange={(e) => setField("phone1", e.target.value)}
                placeholder={t("carCenter.form.phone1Placeholder")}
              />
              {errors.phone1 ? <div className="text-xs text-red-600">{errors.phone1}</div> : null}
            </div>
            <div className="grid gap-2">
              <Label>{t("carCenter.form.phone2")}</Label>
              <Input
                value={draft.phone2}
                onChange={(e) => setField("phone2", e.target.value)}
                placeholder={t("carCenter.form.phone2Placeholder")}
              />
            </div>
            <div className="grid gap-2">
              <Label>{t("carCenter.form.hours")}</Label>
              <Input
                value={draft.hours}
                onChange={(e) => setField("hours", e.target.value)}
                placeholder={t("carCenter.form.hoursPlaceholder")}
              />
              {errors.hours ? <div className="text-xs text-red-600">{errors.hours}</div> : null}
            </div>
            <div className="grid gap-2">
              <Label>{t("carCenter.form.daysOff")}</Label>
              <Select value={draft.daysOff} onChange={(e) => setField("daysOff", e.target.value)}>
                <option value="">{t("sell.common.select")}</option>
                <option value="everyday">{t("carCenter.form.daysOff.everyday")}</option>
                <option value="weekend">{t("carCenter.form.daysOff.weekend")}</option>
                <option value="weekday">{t("carCenter.form.daysOff.weekday")}</option>
              </Select>
            </div>
            <div className="flex items-center gap-2">
              ...
              />
              <Label>{t("carCenter.form.emergency")}</Label>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t("carCenter.form.location")}</CardTitle>
          </CardHeader>
          <CardContent className="grid gap-4">
            <div className="grid gap-2">
              <Label>{t("carCenter.form.address")}</Label>
              <Input
                value={draft.address}
                onChange={(e) => {
                ...
                placeholder={t("carCenter.form.addressPlaceholder")}
              />
              {errors.address ? <div className="text-xs text-red-600">{errors.address}</div> : null}
              {geocodeQuery.isLoading && <div className="text-xs text-zinc-500">{t("common.loading")}</div>}
              {geocodeResults.length > 1 && (
                <div className="grid gap-2 rounded-lg border border-zinc-200 bg-zinc-50 p-2">
                  <div className="text-xs font-medium text-zinc-700">{t("carCenter.form.searchResults")}</div>
              ...
              {geocodeQuery.data && geocodeQuery.data.length === 0 && debouncedAddress.length >= 6 && (
                <div className="text-xs text-zinc-500">{t("carCenter.form.selectAddress")}</div>
              )}
            </div>
            <div className="grid gap-2">
              <Label>{t("carCenter.form.addressDetail")}</Label>
              <Input
                value={draft.addressDetail}
                onChange={(e) => setField("addressDetail", e.target.value)}
                placeholder={t("carCenter.form.addressDetailPlaceholder")}
              />
            </div>
            <div className="grid gap-2">
              <Label>{t("carCenter.form.region")}</Label>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t("carCenter.form.serviceItems")}</CardTitle>
          </CardHeader>
          <CardContent className="grid gap-4">
            {draft.serviceItems.map((item, idx) => (
              <div key={idx} className="grid gap-2 rounded-lg border border-zinc-200 p-3 sm:grid-cols-3">
                <div className="grid gap-2">
                  <Label className="text-xs">{t("carCenter.form.serviceName")}</Label>
                  <Input
                    value={item.name}
                    onChange={(e) => updateServiceItem(idx, "name", e.target.value)}
                    placeholder={t("carCenter.form.serviceName")}
                  />
                </div>
                <div className="grid gap-2">
                  <Label className="text-xs">{t("carCenter.form.servicePrice")}</Label>
                  <Input
                    value={item.priceMnt}
                ...
                <div className="grid gap-2">
                  <Label className="text-xs">{t("carCenter.form.serviceDuration")}</Label>
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      className="shrink-0"
                    >
                      {t("carCenter.form.removeService")}
                    </Button>
                  </div>
                </div>
              </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t("carCenter.form.photos")}</CardTitle>
          </CardHeader>
          <CardContent className="grid gap-4">
            <div className="grid gap-2">
              <Label>{t("sell.common.imageUpload")} ({t("carCenter.form.photosMax")})</Label>
        ...
        <div className="flex items-center justify-between">
          ...
          <Button variant="primary" onClick={onSubmit} disabled={submitting}>
            {submitting ? t("carCenter.form.submitting") : t("carCenter.form.submit")}
          </Button>
        </div>
      </div>
      ...
      <div className="lg:sticky lg:top-6 lg:h-fit">
        <Card>
          <CardHeader>
            <CardTitle>{t("carCenter.form.mapPreview")}</CardTitle>
          </CardHeader>
          <CardContent>
