...
import { fetchCarsList, fetchModelCounts, type CarsSort } from "@/lib/mockApi";
...
export function BuyAllClient({ searchParams }: { searchParams: Record<string, string | string[] | undefined> }) {
  ...
  }, [router, page, sort, modelText, model, yearMin, yearMax, importYearMin, importYearMax, mileageMinKm, mileageMaxKm, priceMinMnt, priceMaxMnt, fuel, transmission, color, regionGroup]);

  const modelCountsQuery = useQuery({
    queryKey: ["cars", "modelCounts", { modelText, yearMin, yearMax, importYearMin, importYearMax, mileageMinKm, mileageMaxKm, priceMinMnt, priceMaxMnt, fuel, transmission, color, regionGroup }],
    queryFn: () =>
      fetchModelCounts({
  ...
  const listQuery = useQuery({
    queryKey: [
      "cars",
      "list",
      { sort, page, modelText, model, yearMin, yearMax, importYearMin, importYearMax, mileageMinKm, mileageMaxKm, priceMinMnt, priceMaxMnt, fuel, transmission, color, regionGroup },
    ],
    queryFn: () =>
      fetchCarsList({
        sort,
